...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing PRINCESS TIARA 
│└────────────┈ ⳹        
│*©2024-2099 King Marisel*
└─────────────────┈ ⳹*
${readmore}`;
  ...
  async (_0x22514a) => {
    try {
      let _0x20ed34 = (
        ...
        "/" +
        formatp(os.totalmem()) +
        "\n\n *🧑‍💻 :*  PRINCESS TIARA ɪꜱ ɴᴏᴡ Aᴠᴀɪʟᴀʙʟᴇ\n\n" +
        readmore +
        "\n╭──❰ *ALL MENU* ❱\n│🏮 Lɪꜱᴛ\n│🏮 Cᴀᴛᴇɢᴏʀʏ\n│🏮 Hᴇʟᴘ\n│🏮 Aʟɪᴠᴇ\n│🏮 Uᴘᴛɪᴍᴇ\n│🏮 Wᴇᴀᴛʜᴇʀ\n│🏮 Lɪɴᴋ\n│🏮 Cᴘᴜ\n│🏮 Rᴇᴘᴏꜱɪᴛᴏʀʏ\n╰─────────────⦁"
      ).trim();
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*Princess Tiara ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `Princess Tiara 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nPRINCESS TIARA`;
        ...
        async () => {
          ...
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `PEINCESS TIARA 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds\n\n*Line:*\n${line.result}\n\nPRINCESS TIARA`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*PRINCESS TIARA ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "PRINCESS TIARA Running Since",
        body: message,
        footerText: "PRINCESS TIARA 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("PRINCESS TIARA_FANCY_TEXT")
        ) {
